...
import 'package:ostadjee/app/common/pojo_model/tutor_login_model.dart';
...
class TutorController extends GetxController {
  ...
  List<String> tutorType = ['Academic', 'Private Teacher', 'Both'];
  ...
  void onLoginClick(bool isSocialLogin) async {
    TutorLoginModel loginModel;

    final _response =
        await FrontEndRepository().postTutorLogin(phone!, password!);

    if (_response.id == ResponseCode.SUCCESSFUL) {
      loginModel =
          TutorLoginModel.fromJson(json.decode(_response.object.toString()));

      Storage.saveValue(Constants.token, loginModel.token);
      Storage.saveValue(Constants.userType, "0");
  ...
  void onVerifyClick() async {
    final _response = await FrontEndRepository().postTutorVerification(
  ...
  void onForgetPasswordClick() async {
    final _response =
        await FrontEndRepository().postTutorForgetPassword(phone!);

    if (_response.id == ResponseCode.SUCCESSFUL) {
      Get.toNamed(Routes.TUTOR_FORGET_PASSWORD_VERIFY);
    } else {
      showErrorToast(_response.object.toString());
    }
  }
  ...
  void onForgetPasswordVerifyClick() async {
    final _response = await FrontEndRepository()
        .postTutorForgetPasswordVerify(token!, password!, confirmPassword!);
    ...
    if (_response.id == ResponseCode.SUCCESSFUL) {
      ...
      Get.offAllNamed(Routes.TUTOR_LOGIN);
    } else {
      showErrorToast(_response.object.toString());
    }
  }
